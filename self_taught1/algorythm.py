...
result = is_anagram("iceman", "nicema")
...
def count_characters(string):
    for c in string:
        if c in count_dict:  # go through each character
            count_dict[c] += 1  # if in count you add 1 to the value of key
        else:
            count_dict[c] = 1  # add to dictionary as key with value of 1
    print(count_dict)  # contains key value pair for each character. key is character
    # and value is number of times it occurred
